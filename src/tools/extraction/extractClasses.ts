...
const init = async () => {
  ...
  const { rules, colorsFromRules, variantsFromRules } = mapClassesIntoRules(uniqueClasses)

  // Extraire les variables SCSS utilisées directement dans le code
  const { colors: colorsFromCode, variants: variantsFromCode } = await getUsedVariables(files)

  // Combiner les deux sources
  const allColors = [...new Set([...colorsFromRules, ...colorsFromCode])]
  const usedVariants = { ...variantsFromRules, ...variantsFromCode }

  // Ajouter les variations "always" export
  const alwaysExportVariants = getAlwaysExportVariants()
  const allVariants = { ...usedVariants, ...alwaysExportVariants }

  log(`${allColors.length} colors generated`)
  log(`${Object.keys(allVariants).length} variants generated`)

  const file = buildCssRuleFile(rules, allColors, allVariants)
  return file
}
...
const getUsedVariables = async (files: string[]): Promise<{ colors: string[], variants: Record<string, string> }> => {
  const extractedVariables = (await Promise.all(
    files.map(async file => extractVariables(file))
  ))
  ?.flat()
...
const findVariantInConfig = (varName: string): string | null => {
  ...
  for (const preset of allPresets) {
    ...
    if (preset.variations) {
      for (const [variantName, variantValue] of Object.entries(preset.variations)) {
